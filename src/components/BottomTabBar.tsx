  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border px-4 py-3" style={{ paddingBottom: `calc(0.75rem + var(--safe-area-inset-bottom))` }}>
      <div className="flex items-center justify-between max-w-md mx-auto">
        ...
        >
          <Plane size={30} className={location.pathname.startsWith("/search") ? "text-primary" : "text-muted-foreground"} />
        </Button>
        ...
        >
          <User size={30} className={location.pathname.startsWith("/account") ? "text-primary" : "text-muted-foreground"} />
        </Button>
      </div>
    </div>
  );
